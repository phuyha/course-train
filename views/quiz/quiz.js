...
async function getCorrectAnswers(questionId) {
    let data = await quizData.getCorrectAnswersByQuestionId(questionId);
    return data;
}
...
ipcRenderer.on('get-data', async (event, courseId, user) => {
    currentUser = user;
    const questions = await getQuestions(courseId);
    
    for (let question of questions) {
        let answers = await getAnswers(question.id);
        let correctAnswers = await getCorrectAnswers(question.id);
        let wrongAnswers = [];
        for (let answer of answers) {
            for (const correctAnswer of correctAnswers) {
                if (answer.id != correctAnswer.correctId) {
                    await wrongAnswers.push(answer);
                }
            }
        }
        ...
        //     if ( answers[i].id == correctAnswers[j].correctId) {
        //         answers.splice(i, 1); 
        //     }
        //  }
        console.log(wrongAnswers);
        all_questions.push = await {
            question_string: question.content,
            choices: {
